...
| `type.css` | `apps/web/src/styles/storytangl-type.css`; `docs/src/_static/storytangl-type.css` | CSS |
...
No path exports are checked in here. Downstream targets that need outlined
...
| Never | Why |
|-------|-----|
| Strip the turned-`G` from the README title because it "looks silly in markdown" | The rotation is the brand. The fix is an SVG banner above the title, not removing the rotation. |
...
The 180° flip predates the name — it carried over from an earlier
worldbuilding project, applied there to a different letter. That's the
whole of it: a footnote, not a selling point. Don't build the brand around
the backstory — and don't "modernize" the flip away either.
