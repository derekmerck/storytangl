...
from tangl34.core.handlers.effect import HasEffects

class TestEffectNode(HasEffects, Node):

    def __init__(self):
        super().__init__()
        self.effects = ["x += 1", "x += 1"]

    @HasEffects.effect_handler(priority=5)
...
def test_apply_effects_increments():
    node = TestEffectNode()
    ctx = {"x": 0, "y": ""}
    TestEffectNode.apply_effects("before", node, ctx=ctx)
    assert ctx["x"] == 2
    assert ctx["y"] == "hello"

def test_effect_order():

    class EffectOrderNode(HasEffects, Node):
        @HasEffects.effect_handler(priority=1)
        def effect1(self, ctx):
            ctx["order"] = ctx.get("order", []) + ["first"]
        @HasEffects.effect_handler(priority=10)
        def effect2(self, ctx):
            ctx["order"] = ctx.get("order", []) + ["second"]
    node = EffectOrderNode()
    ctx = {}
    EffectOrderNode.apply_effects("before", node, node, ctx=ctx)
