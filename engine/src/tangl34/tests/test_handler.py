from tangl34.core.handlers import Handler, HandlerRegistry, ServiceKind
...
def test_handler_priority_ordering():
    h1 = Handler(func=lambda x, y: 1, service=ServiceKind.GATHER, priority=1, label="h1")
    h2 = Handler(func=lambda x, y: 2, service=ServiceKind.GATHER, priority=10, label="h2")
    assert h1 < h2
...
def test_handlerregistry_register_and_iter_handlers():
    ...
    @registry.register(ServiceKind.GATHER, priority=5)
    def handler_a(ent, ctx):
        called.append("A")
        return "A"

    @registry.register(ServiceKind.GATHER, priority=2)
    def handler_b(ent, ctx):
        called.append("B")
        return "B"

    handlers = list(registry.iter_handlers(ServiceKind.GATHER))
...
def test_handlerregistry_find_all_handlers_for(monkeypatch):
    ...
    @registry.register(ServiceKind.GATHER, priority=1, caller_criteria={"foo": 1})
    def handler_c(ent, ctx): return "C"
    # Handler that matches foo=2
    @registry.register(ServiceKind.GATHER, priority=2, caller_criteria={"foo": 2})
    ...
    found1 = registry.find_all_handlers_for(caller1, ServiceKind.GATHER, ctx)
    found2 = registry.find_all_handlers_for(caller2, ServiceKind.GATHER, ctx)
    assert len(found1) == 1
...
def test_handler_mro_tie_break():
    ...
    @reg.register(ServiceKind.GATHER, priority=10, owner_cls=Base)
    def handler_base(ent, ctx): return "base"

    @reg.register(ServiceKind.GATHER, priority=10, owner_cls=Sub)
    ...
    h_sub = reg.find_all_handlers_for(sub_inst, ServiceKind.GATHER, ctx)
    assert h_sub[0].func.__name__ == "handler_sub"
    h_base = reg.find_all_handlers_for(base_inst, ServiceKind.GATHER, ctx)
