...
class HasTraceJournal(Registry[TraceFragment], arbitrary_types_allowed=True):
    """
    ...
    trace_journal: BookmarkedList[UUID] = Field(default_factory=BookmarkedList)
    fragment_counter: int = 0

    def start_journal_section(self, which):
        self.trace_journal.set_bookmark(which, bookmark_type="section")

    def get_journal_section(self, which=-1) -> list[TraceFragment]:
        items = self.trace_journal.get_slice(which, bookmark_type="section")
        return [self.get(uid) for uid in items]

    def add_fragment(self, item: TraceFragment, blame: Node = None):
        if not isinstance(item, TraceFragment):
    ...
    def add_journal_entry(self, items: list[TraceFragment], blame: Node = None):
        if not items:
        ...
        self.trace_journal.add_items([item.uid for item in items],
                               bookmark_name=bookmark_name,
                               bookmark_type="entry")

    def get_journal_entry(self, which=-1) -> list[TraceFragment]:
        # early stop types param looks for terminating section edges, as well
        items = self.trace_journal.get_slice(which, bookmark_type="entry", early_stop_types=["section"])
        return [self.get(uid) for uid in items]
