...
from tangl.core import Entity, Singleton, Registry, TaskHandler
...
class HandlerPipeline(Singleton, HandlerRegistry, Generic[EntityT, ResultT]):
    """
    ...
    **Caching**:
    ...
    def gather_handlers(self,
        ...
        def _sort_handlers(_handlers: list[TaskHandler]) -> list[TaskHandler]:
            ...
            return sorted(_handlers, key=lambda h: (
                ...
                # domain distance is measured by how long the fnmatch is
    ...
    def debug_handlers(self, entity):
        ...
        handlers = self.gather_handlers(entity.__class__, entity.domain)
        for h in handlers:
            logger.info(f"Handler: {h.func.__name__} (Priority={h.priority}, Domain={h.domain}, CallerCls={h.caller_cls})")

    def visualize_pipeline(self):
        import graphviz
        dot = graphviz.Digraph(comment=self.label)
        handlers = sorted(self.handler_registry.items, key=lambda h: (h.priority, h.registration_order))
        for h in handlers:
            node_label = f"{h.func.__name__}\n({h.priority.name}, {h.domain})"
            dot.node(str(h.registration_order), node_label)
        dot.render(f"{self.label}_pipeline.gv", view=True)
