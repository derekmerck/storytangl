...
class TaskHandler(Entity):
    ...
    :class:`HandlerPipeline` for processing :class:`~tangl.core.Entity`
    ...
    **Key Features**:
    ...
    def domain_specificity(self, handler_domain):
        if handler_domain == self.domain:
            return 100  # exact match, highest specificity
        elif handler_domain == '*':
            return 0
        elif fnmatch.fnmatch(self.domain, handler_domain):
            return handler_domain.count('.')  # specificity by segment count
