...
class StructureNode(_FeatureNode):
...
class ResourceNode(_FeatureNode):
...
class ContentFragment(_FeatureNode):
    """
...
class ChoiceEdge(_FeatureEdge[StructureNode, StructureNode]):
    # Links structure nodes
    choice_type: When = "before"

class DependencyEdge(_FeatureEdge[Node, ResourceNode]):
...
class BlameEdge(_FeatureEdge[Node, Node]):
    blame_type: str  # what we are assigning blame for
    
class AssociationEdge(_FeatureEdge[Node, Node]):
