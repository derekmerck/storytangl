...
class ResourceInventoryTag(Entity):
    """
    RITs track data resources, in-mem or on disk.

    RITs for media data can be dereferenced by the response handler at the
    ...
    data_type: ResourceDataType = None

    @model_validator(mode="after")
    def _set_data_type(self):
        if not self.data_type:
            if self.path:
                if data_type := ResourceDataType.from_path(self.path):
