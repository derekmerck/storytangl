...
class GameHandler(ABC, Generic[GameT]):
    ...
    def _get_opponent_move(cls, game: Game):
        if not game.opponent_move_strategy:
            return None
        move = opponent_strategies.execute_first(game, label=game.opponent_move_strategy)
        return move
    ...
    def _revise_opponent_move(cls, game: Game, player_move: Move) -> Move:
        ...
        move = opponent_strategies.execute_first(game, label=game.opponent_revise_move_strategy, player_move=player_move)
        return move
