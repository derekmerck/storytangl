...
# Pre-cutover: reuse legacy endpoint vocabulary so enum identity is preserved.
# Post-cutover: this import points at the same module (self) and fails while the
# module is still initializing, so we fall back to local definitions below.
try:  # pragma: no cover - exercised indirectly across cutover phases
