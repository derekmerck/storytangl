...
class ResponseType(Enum):
    """
    Native response classification for orchestrator endpoints.

    ResponseType defines the category of response returned by a controller.
    The concrete return type is expressed by the function annotation:

    - CONTENT: endpoint returns a ``FragmentStream`` (``list[BaseFragment]``)
    - INFO: endpoint returns an :class:`~tangl.service.response.native_response.InfoModel`
      subclass (``UserInfo``, ``StoryInfo``, etc.)
    - RUNTIME: endpoint returns a :class:`~tangl.service.response.native_response.RuntimeInfo`
    - MEDIA: endpoint returns a ``MediaNative`` (:class:`~tangl.journal.media.MediaFragment`)

    Transport layers wrap or transform these native types for their protocol.
    """
