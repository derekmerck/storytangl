...
def test_file_rit_initialization(temp_fp):
    resource_tag = ResourceInventoryTag(path=temp_fp, label='test', data_type=ResourceDataType.IMAGE)
    assert resource_tag.label == "test"
    assert resource_tag.data_type == ResourceDataType.IMAGE
...
def test_resource_type_inference(temp_image_fp):
    ...
    assert rit.data_type is ResourceDataType.IMAGE
