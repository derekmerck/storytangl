...
def test_effect_handler_builtins():
    ...
    assert r.exec_expr("2+3") == {}, "returns empty context dict"

    # no access for other builtins
    with pytest.raises(NameError):
        r.exec_expr('print("hello")')
...
def test_apply_effects_to():
    ...
    mixin.apply_effects_to(ref_node)
...
def test_conditional_applyable():
    ...
    MyCEEntity = type('MyCEEntity', (HasConditions, HasEffects, Entity), {} )

    conditions = ["'sword' in inventory"]
    effects = ["inventory.remove('sword')", "inventory.append('elixir')"]
    namespace = {"inventory": ['sword', 'shield']}

    mixin = MyCEEntity(locals=namespace, conditions=conditions, effects=effects)
