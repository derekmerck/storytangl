...
from tangl.core.handler import BaseHandler, HandlerRegistry
...
def test_handler_priority_ordering():
    h1 = BaseHandler(func=lambda x, y: 1, priority=1, label="h1")
    h2 = BaseHandler(func=lambda x, y: 2, priority=10, label="h2")
    assert h1 < h2
...
def test_handler_satisfied_with_predicate_and_criteria(monkeypatch):
    ...
    ctx = {"x": 42}
    # BaseHandler with caller_criteria foo=1, predicate always_true
    h = BaseHandler(
        func=lambda x, y: 1,
        priority=0,
    ...
    # BaseHandler with caller_criteria foo=2, will NOT match
    h2 = BaseHandler(
        func=lambda x, y: 1,
        priority=0,
    ...
    # BaseHandler with predicate always_false
    h3 = BaseHandler(
        func=lambda x, y: 1,
        priority=0,
        caller_criteria={"foo": 1},
