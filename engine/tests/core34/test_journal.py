...
from tangl34.core.trace import HasTraceJournal, TraceFragment

class TestHasJournal(HasTraceJournal):
    # Inherit everything, no change needed for test
    pass

def make_fragments(n, kind="text"):
    return [TraceFragment(uid=uuid4(),
                            fragment_type=kind,
                            label=f"Frag {i}",
...
def test_add_single_item():
    ...
    journal_graph.trace_journal.add_item("first")
    assert len(journal_graph.trace_journal) == 1
    assert journal_graph.trace_journal.get_items() == ["first"]
...
def test_add_single_entry():
    ...
    assert len(journal_graph.trace_journal) == 1
    for f in frags:
        assert f.uid in journal_graph.trace_journal

    assert 'entry' in [b.bookmark_type for b in journal_graph.trace_journal._bookmarks], f"Bookmark type 'entry' should exist in {journal_graph.trace_journal._bookmarks}"

    assert journal_graph.trace_journal.get_slice(bookmark_type="abc") == []
    last_entry = journal_graph.trace_journal.get_slice(bookmark_type="entry")
...
def test_add_and_retrieve_entry():
    ...
    assert len(journal_graph.trace_journal) == 3
    for f in frags:
        assert f.uid in journal_graph.trace_journal, "Fragments should be in view"

    # Entries should be retrievable
    entry = journal_graph.get_journal_entry()
    assert len(entry) == 3
    assert all(isinstance(f, TraceFragment) for f in entry)
...
def test_empty_journal():
    ...
    assert len(journal_graph.trace_journal) == 0
    assert journal_graph.trace_journal.get_items() == []
    assert journal_graph.trace_journal.get_bookmarks() == []
