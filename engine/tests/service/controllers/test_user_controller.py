"""Service UserController orchestrator/gateway flow tests.
...
methods directly. These tests assert the *service-specific* concerns:

  - The service UserController wrapper exposes ApiEndpoint metadata with
...
are not re-testing the legacy logic but the service dispatch plumbing.
"""
...
class TestUserControllerGatewayRouting:
    ...
    def test_gateway_module_binding_is_service_user_controller(
        self,
        orchestrator: Orchestrator,
    ) -> None:
        """service bootstrap wires the service UserController, not the legacy one."""
        endpoint_name = endpoint_for_operation(ServiceOperation.USER_CREATE)
