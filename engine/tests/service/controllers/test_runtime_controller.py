"""Service RuntimeController orchestrator/gateway flow tests.

This is the service replacement for the PORT_ADAPT row:
...
These tests assert the service-specific contracts:

- ApiEndpoint binding metadata for story runtime endpoints.
- Orchestrator hydration and persistence policy behavior for create/do/drop.
- Gateway operation routing for story info/update operations.
"""
...
def _story_script() -> dict[str, Any]:
    return {
        "label": "svc_world",
        "metadata": {"title": "Svc Runtime", "author": "Tests", "start_at": "intro.start"},
        "scenes": {
            "intro": {
                "blocks": {
                    "start": {
...
@pytest.fixture
def world() -> World:
    return World.from_script_data(script_data=_story_script())
...
def test_story_operation_tokens_route_to_registered_runtime_endpoints(
...
def test_resolve_choice_rejects_legacy_passback_param(
    ...
    created = orchestrator.execute(
        ...
        world=world,
        init_mode=InitMode.EAGER.value,
        story_label="svc_story_payload_only",
    )
