...
def test_scene_template_selection_criteria_parent_label():
    template = BaseScriptItem(label="scene-template", scope=ScopeSelector(parent_label="scene1"))

    assert template.get_selection_criteria() == {"has_parent_label": "scene1"}
...
def test_complex_template_selection_criteria():
    ...
    template = BaseScriptItem(label="complex", scope=scope)

    assert template.get_selection_criteria() == {
        "has_parent_label": "scene1",
        "has_ancestor_labels": {"world"},
