...
methods directly. These tests assert the *service38-specific* concerns:
  ...
  - The service38 UserController wrapper exposes ApiEndpoint38 metadata with
  ...
  - The gateway flow (ServiceGateway38.execute_endpoint) routes USER_CREATE and
...
class TestUserControllerBindingMetadata:
    """ApiEndpoint38.binds annotations are correct for each endpoint."""

    def _ep(self, name: str) -> ApiEndpoint38:
        endpoints = UserController.get_api_endpoints()
        ep = endpoints[name]
        assert isinstance(ep, ApiEndpoint38), f"{name} is not an ApiEndpoint38"
        return ep
    ...
    def test_all_endpoints_are_api_endpoint38_instances(self) -> None:
        for name, ep in UserController.get_api_endpoints().items():
            assert isinstance(ep, ApiEndpoint38), (
                f"UserController.{name} is not an ApiEndpoint38 instance"
            )
...
class TestCreateUserOrchestrator:
    def test_create_user_returns_runtime_info(self, orchestrator: Orchestrator38) -> None:
        result = orchestrator.execute("UserController.create_user", secret="dev-secret")
        assert isinstance(result, RuntimeInfo)
        assert result.status == "ok"

    def test_create_user_details_has_user_and_user_id(self, orchestrator: Orchestrator38) -> None:
    ...
    def test_create_user_result_user_id_is_valid_uuid(self, orchestrator: Orchestrator38) -> None:
...
class TestUserControllerGatewayRouting:
    ...
    def test_user_info_operation_routes_to_user_controller(
        ...
        endpoint_name = endpoint_for_operation(ServiceOperation38.USER_INFO)
        assert endpoint_name in orchestrator._endpoints
    ...
    def test_user_drop_operation_routes_to_user_controller(
        ...
        endpoint_name = endpoint_for_operation(ServiceOperation38.USER_DROP)
        assert endpoint_name in orchestrator._endpoints
    ...
    def test_gateway_create_user_returns_runtime_info(
        ...
        result = gateway.execute(ServiceOperation38.USER_CREATE, secret="gw-secret")
        assert isinstance(result, RuntimeInfo)
        assert result.status == "ok"
