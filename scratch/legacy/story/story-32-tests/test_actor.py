...
def test_actor2():
    ...
    assert actor in graph.nodes.values(), "Actor not in graph.nodes"
    assert role in graph.nodes.values(), "Role not in graph.nodes"
...
def test_multiple_role_casting():
    ...
    assert len( graph.find_nodes(Actor) ) == 2
