...
def resolve(node: Node, graph: Graph, reg: ProviderRegistry, cap_cache: HandlerCache, ctx: StringMap):
    ...
    def _find_or_create(req: Requirement,
                        scope_tier: CoreScope,
                        reg: ProviderRegistry,
                        ctx: StringMap) -> ProviderCap | None:
        """Return an existing provider that satisfies *req* or create one."""
        # 1. search outward
        for CoreScope in CoreScope.range_outwards(scope_tier):  # NODE → … → GLOBAL
            for prov in reg.providers(req.key, CoreScope):
    ...
    search_chain = [CoreScope.NODE]                               # 1. node‑local
    search_chain.extend([CoreScope.ANCESTORS] * len(list(node.iter_ancestors(graph=graph))))  # 2. each ancestor
    search_chain.extend([CoreScope.GRAPH, CoreScope.DOMAIN, CoreScope.GLOBAL])                   # 3‑5
    ...
    for req in getattr(node, "requires", []):
        ...
        for CoreScope in search_chain:
            cap = _find_or_create(req, CoreScope, reg, ctx)
            if cap:
                break
