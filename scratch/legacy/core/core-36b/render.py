...
class Renderable(HasContext):
    ...
    @classmethod
    def render_content(cls, caller, *objects, ctx) -> list[TraceFragment]:
        logger.debug("rendering content")
        fragments = []  # type: list[TraceFragment]
        ...
        return [TraceFragment.structure(data) for data in fragments]
