...
| `progression/` | **Audited** — see `progression/AUDIT.md`. Three generations with literal duplicates (`task.py` and `task-2.py`), but promotion is far more complete than the file count suggests: the realized package is a rewrite, not a thin promotion. `Quality`, `SituationalEffect`, and the LogInt/probit handlers are implemented, exported, tested, and consumed by the `coronate_the_regent` world. `StatCurrency` and the hardcoded stat domains are superseded by `StatDef.currency_name` and `StatSystemDefinition` + `presets/`. Four badge-related sources are retained; ~42 files are recommended for retirement. Umbrella #112. |
