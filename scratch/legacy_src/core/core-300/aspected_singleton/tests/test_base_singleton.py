...
class SimpleSingleton(Singleton):
    ...
    @staticmethod
    def _filter_by_value(inst: SimpleSingleton, value: str) -> bool:
...
class PydanticSingleton(Singleton, BaseModel):
    ...
    # @staticmethod
    # def _filter_by_value(inst: PydanticSingleton, value: str) -> bool:
    #     return inst.value == value
...
def test_singleton_uniqueness():
    ...
    s1 = SimpleSingleton(value="test")
    s2 = SimpleSingleton(value="test")
    assert s1.uid == s2.uid
    assert s1 is s2
    assert len(SimpleSingleton._instances) == 1


def test_different_values_different_instances():
    """Test that different values create different instances"""
    s1 = SimpleSingleton(value="test1")
    s2 = SimpleSingleton(value="test2")
    assert s1.uid != s2.uid
    assert s1 is not s2
    assert len(SimpleSingleton._instances) == 2
...
def test_get_instance():
    s1 = SimpleSingleton(value="test")
    s2 = SimpleSingleton.get_instance(s1.uid)
    assert s1 is s2


def test_get_instance_missing():
    with pytest.raises(KeyError):
        SimpleSingleton.get_instance(UUID(int=0))
...
def test_subclass_instances():
    ...
    parent = SimpleSingleton(value="test")
    child = ChildSingleton(value="test")

    assert parent.uid == child.uid  # Same digest
...
def test_search_subclasses():
    child = ChildSingleton(value="test")
    found = SimpleSingleton.get_instance(child.uid, search_subclasses=True)
...
def test_pydantic_singleton():
    ...
    s1 = PydanticSingleton(value="test")
    s2 = PydanticSingleton(value="test")
    assert s1 is s2
    assert s1.model_dump() == {"obj_cls": PydanticSingleton, "uid": s1.uid}


def test_pydantic_validation():
    """Test that Pydantic validation still works"""
    with pytest.raises(ValueError):
        PydanticSingleton()  # Missing required field


def test_pydantic_optional_fields():
    """Test optional fields behave correctly"""
    s1 = PydanticSingleton(value="test", optional="opt")
    s2 = PydanticSingleton(value="test")  # Same identity even with different optional
    assert s1 is s2
...
def test_pydantic_immutability():
    ...
    s = PydanticSingleton(value="test")
...
def test_pickle_singleton():
    ...
    original = SimpleSingleton(value="test")
...
def test_clear_instances():
    ...
    SimpleSingleton(value="test1")
    SimpleSingleton(value="test2")
    ChildSingleton(value="test3")

    SimpleSingleton.clear_instances(clear_subclasses=True)
    assert len(SimpleSingleton._instances) == 0
...
def test_duplicate_registration_error():
    ...
    s = SimpleSingleton(value="test")

    # Try to register same instance again
    with pytest.raises(KeyError):
        SimpleSingleton.register_instance(s)
...
def test_find_instances():
    ...
    found = SimpleSingleton.find_instances(value="test1")
    assert len(found) == 1
    assert found[0] is s1

    found = SimpleSingleton.find_instance(value="test2")
    assert found is s2
...
def test_singleton_pydantic_initialization():
    ...
    class PydanticSingleton(Singleton, InitCounting, BaseModel):
        value: str
    ...
    #     PydanticSingleton(value=123)  # wrong type
    # assert new_counter == 2  # __new__ still called
    # assert init_counter == 1  # but __init__ wasn't
...
def cleanup():
    ...
    PydanticSingleton.clear_instances()
    yield
    SimpleSingleton.clear_instances()
