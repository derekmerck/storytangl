// v12-sections.jsx — the long-form sections of the wireframe doc.
// Each top-level export is a React component used in v12-app.jsx.
...
// ===========================================================================
// What changed in v1.2 (rename ledger + summary)
// ===========================================================================
...
function V12Changes() {
  return (
    <div className="vocab-changes">
      <div className="ch">
        <span className="from">accepts.kind: "tokens"</span><span className="arrow">→</span><span className="to">"pieces"</span>
        <div className="muted">Mirrors <code>piece_ids</code> payload + <code>PieceFragment</code>. (A v1.2 draft proposed <code>"select"</code>; reverted in v1.3 — <code>select</code> stays reserved for a future generic selection accepts.)</div>
      </div>
      <div className="ch">
        <span className="from">token</span><span className="arrow">→</span><span className="to">piece</span>
        <div className="muted">UI atom rename; engine `Token` keeps its name.</div>
      </div>
      <div className="ch">
        <span className="from">token_offer</span><span className="arrow">→</span><span className="to">piece (realized=false)</span>
        <div className="muted">One fragment type covers both offers and minted pieces.</div>
      </div>
      <div className="ch">
        <span className="from">dice_roll content_format</span><span className="arrow">→</span><span className="to">RollFragment</span>
        <div className="muted">Generalizes to card draws, random tables, etc.</div>
      </div>
      <div className="ch">
        <span className="from">choice_id</span><span className="arrow">→</span><span className="to">edge_id</span>
        <div className="muted">Reconciles with `ChoiceFragment.edge_id`. HTTP body too.</div>
      </div>
      <div className="ch">
        <span className="from">cost_preview</span><span className="arrow">→</span><span className="to">cost_previews[]</span>
        <div className="muted">Multi-axis costs are common (money + time + reputation).</div>
      </div>
      <div className="ch">
        <span className="from">ledger value_type</span><span className="arrow">→</span><span className="to">annotated kv_list</span>
        <div className="muted">Populated KvRows absorb bars / fractions / deltas.</div>
      </div>
      <div className="ch">
        <span className="to">+ InterpretationFragment</span>
        <div className="muted">P1. Backend feedback for raw_command; does NOT advance step.</div>
      </div>
      <div className="ch">
        <span className="to">+ PieceFragment.owner · position</span>
        <div className="muted">§7.1. <code>owner</code> = cursor whose channel owns the piece (visibility/audience). <code>position</code> = spatial binding inside parent zone (<code>{`{x,y}`}</code> · <code>{`{row,col}`}</code> · <code>{`{q,r}`}</code>).</div>
      </div>
      <div className="ch">
        <span className="to">+ GraphLayout.edges[] with uids</span>
        <div className="muted">§7.2. Adjacencies are first-class addressable targets via <code>PlaceAccepts.edge_ref</code> (route-building games).</div>
      </div>
      <div className="ch">
      ...
      <div className="ch">
        ...
        <div className="muted">L1 vocab (this doc) · L2 API · L3 engine. Spec is target-truth. Per-surface implementation status tracked in <code>WIDGET_CONTRACT_RECONCILIATION.md</code>; layers may be at different states during settling.</div>
      </div>
      <div className="ch">
        <span className="from">§1.5 + §1.6 = Tier S</span><span className="arrow">→</span><span className="to">Tier P1</span>
      ...
      <div className="ch">
        <span className="to">~ owner · position · edge_ref · audience visibility</span>
        <div className="muted">Now framed as <b>proposal fixtures</b> awaiting bundle MVP authors — not gating fixtures for current conformance.</div>
      </div>
    </div>
  );
}
...
function FragmentMatrix() {
  const rows = [
    ...
    { frag: "content",            widget: "Prose block",       scroll: "inline paragraph",   dossier: "stage prose",       stage: "caption (current) + log row" },
    { frag: "attributed",         widget: "Dialog line",       scroll: "indented line w/ avatar", dossier: "bracketed",     stage: "active caption" },
    ...
    { frag: "media · avatar_im / dialog_im", widget: "Avatar / inline figure", scroll: "attached to line", dossier: "attached to line", stage: "sprite + lower-left inset" },
    ...
    { frag: "interpretation",     widget: "Transcript line",   scroll: "inline w/ accent",   dossier: "inline w/ accent",  stage: "log row", tier: "P1" },
    { frag: "piece",              widget: "Piece chip",        scroll: "(in zone)",          dossier: "(in zone)",         stage: "(in zone)", tier: "P2" },
    { frag: "group · zone",       widget: "Zone tile (slot / hand / field / catalog / pile)", scroll: "stacked tiles", dossier: "tiles in stage", stage: "HUD strip", tier: "P2" },
    { frag: "roll",               widget: "Dice ritual + outcome chip", scroll: "inline panel", dossier: "inline panel",   stage: "log row + skip", tier: "P2" },
    ...
    { frag: "kv_list (+ KvRow)",  widget: "Key-value table; bars/fractions/deltas via KvRow.hint", scroll: "wrapped strip", dossier: "rail rows", stage: "HUD stack" },
  ...
  return (
    <div className="matrix-wrap">
      <table className="matrix">
        <thead>
          <tr>
            <th style={{width:"22%"}}>Fragment / value_type</th>
...
function GarageDemo() {
  const fx = window.V12_FIXTURES.garage;
  const env = fx.envelope;
...
function RouteBuildingDemo() {
  ...
  // the v1.3 spec — proposal fixture, not gating).
  const cities = [
...
function RollDemo() {
  const fx = window.V12_FIXTURES.gravel;
...
function CommandDemo() {
  const env = window.V12_FIXTURES.crossroads.envelope;
  const samples = window.V12_FIXTURES.gravel.interp_samples;
  return (
    <div className="panel-card">
...
// ===========================================================================
// §10 — SANDBOX demo (the new v1.2 conventions; primary integration)
// ===========================================================================

function SandboxDemo() {
  const fx = window.V12_FIXTURES.manor;
  const env = fx.envelope;
  const proj = fx.projected_state;
  const idx = indexEnvelope(env);
  const byUid = idx.byUid;
...
function ParityTable12() {
  const rows = [
    ...
    { w: "info-affordance bar",     web: "pill row · dirty marks",   cli: "m) map  i) inv  t) time",          tk: "Frame of Buttons",          renpy: "stats screen affordances" },
...
Object.assign(window, {
  TierLegend, ParityStool, V12Changes,
